...
if(!file.exists("S:/AG/AG-CSB_NeuroRad2/khalila/PROJECTS/BD_REPRO/ANALYSIS/temp_df_subs_reldiffs.csv")) {
...
for(i in subs) {
  ...
  write.csv(temp_df_subs, "S:/AG/AG-CSB_NeuroRad2/khalila/PROJECTS/BD_REPRO/ANALYSIS/temp_df_subs_reldiffs.csv")
   }} else {
    # load in prepepared data
      temp_df_subs <- read.csv("S:/AG/AG-CSB_NeuroRad2/khalila/PROJECTS/BD_REPRO/ANALYSIS/temp_df_subs_reldiffs.csv")
     demogr <- read.csv2(file = paste("S:/AG/AG-CSB_NeuroRad2/khalila/PROJECTS/BD_REPRO/ANALYSIS/demographics.csv", sep=";"), header = T)
   }
...
plot(temp_df_subs[temp_df_subs$cens == "nocens",]$D0, temp_df_subs[temp_df_subs$cens == "nocens",]$diffvals); abline(coef=c(0,1),col="red")
plot(temp_df_subs[temp_df_subs$cens == "nocens",]$D1, temp_df_subs[temp_df_subs$cens == "nocens",]$diffvals); abline(coef=c(0,-1),col="red")

# for censored data
plot(temp_df_subs[temp_df_subs$cens == "cens",]$D0, temp_df_subs[temp_df_subs$cens == "cens",]$D1); abline(coef=c(0,1),col="red")
plot(temp_df_subs[temp_df_subs$cens == "cens",]$D0, temp_df_subs[temp_df_subs$cens == "cens",]$diffvals); abline(coef=c(0,1),col="red")
plot(temp_df_subs[temp_df_subs$cens == "cens",]$D1, temp_df_subs[temp_df_subs$cens == "cens",]$diffvals); abline(coef=c(0,-1),col="red")
par(mfrow=c(1,1))
```
...
model1 <- lmer(diffvals ~ D0 + vt + type + I(FDmean*10) + tr + field + I(int/10) + I(length/600) + cens + (1|subject) + (1|study), data = temp_df_subs)
...
model3 <- glmmPQL(diffvals ~ D0 + vt + type + I(FDmean*10) + tr + field + I(int/10) + I(length/600) + cens, random=list(study = ~1, subject = ~1), family = gaussian(link="identity"), data = temp_df_subs, verbose = FALSE)
